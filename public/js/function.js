...
(function ($) {
    ...
    function initialize() {
        ...
        var mapOptions = {
            zoom: 16,
            center: new google.maps.LatLng(50.775018, 2.21411),
        ...
        var marker = new google.maps.Marker({
            position: new google.maps.LatLng(50.775018, 2.21411),
            map: map,
            icon: image
        });

        var url = "https://www.google.fr/maps/place/Najeti+H%C3%B4tel+Ch%C3%A2teau+Tilques/@50.775018,2.2119213,17z/data=!3m1!4b1!4m5!3m4!1s0x47dc513bfaae1fdf:0x1af406b1682037a6!8m2!3d50.775018!4d2.21411";

        var infowindow = new google.maps.InfoWindow({
            content: "<div style='padding-left:10px;'>" +
            "<h3 class='force-police text-center' style='margin:5px !important;padding:0px;'>Chateau de Tilques</h3>" +
            "<div style='text-align:center;'><a href='" + url + "'><img src='/assets/images/hotel-chateau-tilques.jpg' style='max-height:100px;'/></a></div>" +
            "<div class='text-center' style='margin-top:5px;'><a class='btn btn-default btn-rsvp' target='_blank' href='" + url + "'>Voir sur la carte</a></div>" +
            "</div>"
        });
        infowindow.open(map, marker);

        map.mapTypes.set(MY_MAPTYPE_ID, customMapType);
        map.setCenter(new google.maps.LatLng(50.7765377, 2.215));
    }

    google.maps.event.addDomListener(window, 'load', initialize);

    $('#rsvp-form').submit(function (event) {
        $('#alertinfo, #alertdanger').text("").addClass('hidden');
        event.preventDefault();
        var response = {
            status: $('#rsvp-status').val(),
            guest_count: parseInt($('#rsvp-guest').val()),
            message: $('#rsvp-message').val()
        };
        $.post('/api/rsvp', JSON.stringify(response)).success(function () {
            $('#alertinfo').removeClass('hidden').text("Votre réponse a bien été prise en compte");
        }).fail(function () {
            $('#alertdanger').removeClass('hidden').text("Une erreur s'est produite, veuillez réessayer ultérieurement");
        })
    });
